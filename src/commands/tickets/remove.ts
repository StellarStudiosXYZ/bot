...
export async function handleTicketRemove(
    ...
    if (!isStaff) {
        return interaction.editReply(
            errorContainer("Only staff can remove users from tickets."),
        );
    }
                ...
                .addSectionComponents(
                    new SectionBuilder()
                        .addTextDisplayComponents(
                            (t) => t.setContent(`### User Removed from Ticket`),
                            (t) =>
                                t.setContent(
                                    `<:member:1315248772527423551> **User Removed**\n${targetUser} - \`${targetUser.id}\``,
                                ),
                            (t) =>
                                t.setContent(
                                    `<:member:1315248772527423551> **Removed By**\n${interaction.user} - \`${interaction.user.id}\``,
                                ),
                        )
    ...
    logger.info(
        `[TICKET_MEMBER_REMOVE] ${targetUser.username} (${targetUser.id}) was removed from ticket #${ticket.id} by ${interaction.user.username} (${interaction.user.id}).`,
    );

    return interaction.followUp(
        successContainer(
            `${targetUser.tag} was removed from ticket <#${ticket.channelId}>.`,
        ),
    );
}
